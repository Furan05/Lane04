...
### Garmin — relais auto-hébergé (décision operator, amendement « sans serveur »)
- **Décision operator** : ouvrir une voie **Garmin** (envoi des protocoles vers une montre Garmin, à terme). Le principe fondateur devient « **sans serveur imposé** » : la voie Apple Watch reste 100 % locale ; la voie Garmin exige un relais parce que la Garmin Training API impose OAuth + un client secret qui ne peut pas vivre dans une app gratuite. Le relais est **auto-hébergeable par l'athlète** (`backend/`, Node ≥ 18, zéro dépendance) — pas de serveur LANE 04 central, pas de monétisation.
- **Client iOS** : `Lane04/Data/GarminIntegration.swift` — `ASWebAuthenticationSession` → `lane04://garmin/oauth/callback` (host `garmin`, path `/oauth/callback`), `connection_token` opaque en Keychain, **jamais de secret Garmin dans l'app**. `publish(proto:vma:at:)` passe par `ProtocolValidator` (même ligne de défense que WorkoutKit) — **prêt mais pas branché dans le flux INJECT** (décision UX ouverte, voir NEXT_UPDATE.md).
- **Relais** : `backend/` — OAuth2 PKCE, jetons Garmin côté serveur (connection_token hashé SHA-256), mapper avec **les bornes exactes de `ProtocolValidator`**, fourchettes d'allure ±2.5 pts %VMA (miroir `VMACalculator`). **Vérité de transmission** : workout créé mais schedule en faute → `201 { scheduled:false }`, jamais une erreur qui ferait réinjecter (doublon). Endpoints Garmin par défaut **à confronter à la spec du portail développeur** (surchargeables par env).
- **CONSOLE** : carte GARMIN — `GARMIN RELAY` (URL du relais, `@AppStorage SettingsKey.garminBackendURL`, masque la ligne LINK tant que vide) + `GARMIN LINK` (`[LINKED]`/`[NO LINK]`, faute nommée + RETRY). Aucun aplat d'accent (signal, pas décor).
- **Contrat JSON** app→relais figé par `Lane04Tests/GarminPayloadTests.swift` + `backend/test/mapper.test.mjs` (5 tests, `npm test`). `scheduledDate` = jour calendaire **local** `yyyy-MM-dd` (le schedule Garmin est un jour, pas un instant).
